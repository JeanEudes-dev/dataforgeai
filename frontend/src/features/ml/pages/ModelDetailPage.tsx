...
export function ModelDetailPage() {
  ...
  const cvScoresData =
    model.cross_val_scores?.map((score, index) => ({
      fold: `Fold ${index + 1}`,
      score: score,
    })) || [];
  ...
  const confusionMatrix = model.metrics?.confusion_matrix as
    | number[][]
    | undefined;
  const confusionLabels = model.metrics?.confusion_matrix_labels as
    | string[]
    | undefined;

  const rocCurve = model.metrics?.roc_curve as
    | { fpr: number[]; tpr: number[]; thresholds: number[] }
    | undefined;
  const rocData =
    rocCurve?.fpr?.map((fpr, i) => ({
      fpr,
      tpr: rocCurve.tpr[i],
    })) || [];
  ...
  const inputSchema = model.input_schema as
    | Record<string, { dtype: string; nullable: boolean }>
    | undefined;
  ...
  return (
    <div className="space-y-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {model.is_best && (
                    <span className="flex items-center gap-1 px-2 py-1 text-xs font-semibold rounded-full bg-primary/20text-muted-foreground">
                ...
                <p className="text-sm text-muted-foreground">
                  {isClassification ? "Classification" : "Regression"} • Target:{" "}
                  {model.target_column}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 rounded-xl border border-dashed border-border bg-card/80 px-4 py-3 shadow-[0_1px_0_rgba(0,0,0,0.04)]">
              ...
              <div>
                ...
                <p className="text-base font-semibold text-foreground">
                  {formatNumber(model.dataset?.row_count ?? 0, 0)} /{" "}
                  {formatNumber(model.dataset?.column_count ?? 0, 0)}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-xl border border-dashed border-border bg-card/80 px-4 py-3 shadow-[0_1px_0_rgba(0,0,0,0.04)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {isClassification ? (
          <>
            <StatCard
              label="Accuracy"
              value={`${formatNumber((model.metrics?.accuracy || 0) * 100, 1)}%`}
            />
            <StatCard
              label="Precision"
              value={`${formatNumber((model.metrics?.precision || 0) * 100, 1)}%`}
            />
            <StatCard
              label="Recall"
              value={`${formatNumber((model.metrics?.recall || 0) * 100, 1)}%`}
            />
            <StatCard
              label="F1 score"
              value={`${formatNumber((model.metrics?.f1 || model.metrics?.f1_score || 0) * 100, 1)}%`}
            />
          </>
        ) : (
          <>
            ...
            <StatCard
              label="RMSE"
              value={formatNumber(model.metrics?.rmse, 4)}
            />
            <StatCard label="MSE" value={formatNumber(model.metrics?.mse, 4)} />
            <StatCard
              label="R^2"
              value={`${formatNumber((model.metrics?.r2 || model.metrics?.r2_score || 0) * 100, 1)}%`}
            />
          </>
        )}
      </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr>
                      ...
                      <th
                        colSpan={confusionLabels.length}
                        className="p-2 text-sm text-center text-muted-foreground font-medium"
                      >
                        Predicted
                      </th>
                    </tr>
                    <tr>
                      ...
                      {confusionLabels.map((label) => (
                        <th
                          key={label}
                          className="p-2 text-sm text-center text-muted-foreground font-medium min-w-[60px]"
                        >
                          {label}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {confusionMatrix.map((row, i) => {
                      ...
                      return (
                        <tr key={i}>
                          <td className="p-2 text-sm text-muted-foreground font-medium text-right pr-3">
                            {confusionLabels[i]}
                          </td>
                          {row.map((val, j) => {
                            const isCorrect = i === j;
                            ...
                            return (
                              <td
                                ...
                                className={cn(
                                  ...
                                  isCorrect
                                    ? "bg-primary/10text-muted-foreground"
                                    : val > 0
                                ...
                                style={{
                                  opacity:
                                    val > 0 ? Math.max(0.45, intensity) : 1,
                                }}
                              >
                                {val}
                                {rowTotal > 0 && (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      labelFormatter={(fpr) =>
                        `FPR: ${formatNumber(fpr as number, 3)}`
                      }
                      contentStyle={{
                        background: "#fff",
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <ReferenceLine
                      ...
                      stroke="#f59e0b"
                      strokeDasharray="5 5"
                      label={{ value: "Mean", fill: "#f59e0b", fontSize: 12 }}
                    />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border border-border shadow-sm">
          ...
          <CardContent className="space-y-3">
            ...
            <InfoRow
              label="Model size"
              value={model.model_size_display || "-"}
            />
                ...
                <dl className="space-y-3">
                  {Object.entries(model.hyperparameters).map(([key, value]) => (
                    <div key={key} className="flex justify-between">
                      ...
                      <dd className="text-foreground font-medium font-mono text-sm">
      ...
      <Card className="border border-border shadow-sm">
        <CardHeader className="border-b border-border bg-muted/30">
          <CardTitle>
            Feature columns ({model.feature_columns?.length || 0})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {model.feature_columns?.map((col) => (
              <span
                key={col}
                className="px-3 py-1.5 text-sm rounded-full border border-border bg-card text-muted-foreground shadow-[0_1px_0_rgba(0,0,0,0.04)]"
              >
                {col}
              </span>
            ))}
