...
import { Card, CardHeader, CardTitle, CardContent, MetricTooltip, METRIC_DESCRIPTIONS } from '@/components/ui'
import {
  StatCard,
...
export function ModelPerformanceSection({ report }: ModelPerformanceSectionProps) {
  ...
  const rocData = metrics.roc_curve || (metrics.fpr && metrics.tpr ? { fpr: metrics.fpr, tpr: metrics.tpr } : null)
    ...
    >
      ...
      <Card className="border border-gray-200 shadow-sm">
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <InfoTile label="Algorithm" value={model.algorithm_type?.replace(/_/g, ' ')} />
            <InfoTile label="Task" value={model.task_type} />
            <InfoTile
              label="Primary metric"
              value={model.primary_metric !== undefined ? `${(model.primary_metric * 100).toFixed(2)}%` : 'N/A'}
              highlight
            />
            <InfoTile label="Model size" value={model.model_size_display || 'N/A'} />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {displayMetrics.map(({ key, label, format }) => {
          const value = metrics[key as keyof typeof metrics] as number | undefined
      ...
      {model.cross_val_scores && model.cross_val_scores.length > 0 && (
        (() => {
          const cvMean = model.cross_val_scores.reduce((a, b) => a + b, 0) / model.cross_val_scores.length
          const cvStd = Math.sqrt(model.cross_val_scores.map(x => Math.pow(x - cvMean, 2)).reduce((a, b) => a + b, 0) / model.cross_val_scores.length)
          return (
        <Card className="border border-gray-200 shadow-sm">
          <CardHeader className="border-b border-gray-200 bg-gray-50 flex items-center justify-between">
            <div>
              <CardTitle className="text-base">Cross-validation scores</CardTitle>
              <p className="text-xs text-gray-500 mt-1">
                {isClassification ? 'F1 score across folds' : 'R2 score across folds'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {Object.entries(model.hyperparameters).map(([key, value]) => (
                <div key={key} className="p-3 rounded-lg border border-gray-200 bg-white">
                  <div className="text-xs text-gray-500 truncate" title={key}>
                    {key.replace(/_/g, ' ')}
                  </div>
                  <div className="text-sm font-mono text-gray-900 truncate" title={String(value)}>
                    {String(value)}
                  </div>
                </div>
              ))}
...
function InfoTile({ label, value, highlight }: { label: string; value: string | number | undefined; highlight?: boolean }) {
  return (
    <div className="p-3 rounded-xl border border-gray-200 bg-white shadow-[0_4px_12px_-8px_rgba(0,0,0,0.12)]">
      <p className="text-xs text-gray-500 mb-1">{label}</p>
      <p className={highlight ? 'text-sm font-semibold text-primary-700' : 'text-sm font-semibold text-gray-900'}>
        {value ?? 'N/A'}
      </p>
    </div>
  )
