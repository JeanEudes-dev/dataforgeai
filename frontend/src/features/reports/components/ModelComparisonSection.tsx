...
import { Card, CardHeader, CardTitle, CardContent, Tooltip, METRIC_DESCRIPTIONS } from '@/components/ui'
...
type SortKey = 'name' | 'algorithm_type' | 'primary_metric' | 'accuracy' | 'f1' | 'r2' | 'rmse'
type SortDirection = 'asc' | 'desc'

export function ModelComparisonSection({ report }: ModelComparisonSectionProps) {
                        ...
                        <Tooltip
                          ...
                          content={
                            <div className="min-w-[150px]">
                              <div className="font-semibold mb-1">{metricInfo.title}</div>
                              <div className="text-gray-300 text-[11px]">{metricInfo.description}</div>
                            </div>
                          }
                          position="top"
                        >
                ...
                {sortedModels.map((model, index) => (
                  ...
                  >
                    ...
                    {metricOptions.map(({ key }) => {
                      const value = model.metrics?.[key as keyof typeof model.metrics] as number | undefined
                      const isError = ['rmse', 'mae', 'mse', 'mape'].includes(key)
                      return (
                        <td key={key} className="px-4 py-3 text-gray-600 dark:text-gray-400">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4 className="font-semibold text-green-800 dark:text-green-200">
                  Best Model: {sortedModels.find((m) => m.is_best)?.display_name}
                </h4>
                <p className="text-sm text-green-700 dark:text-green-300">
                  Selected based on {isClassification ? 'F1 Score' : 'R2 Score'} performance with cross-validation
                </p>
              </div>
            </div>
          </CardContent>
